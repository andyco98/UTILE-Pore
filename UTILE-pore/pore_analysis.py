...
def calculate_psd(filepath, csv_file, voxel_size=5):  # Added voxel_size parameter with a default of 5 microns
    ...
    ax.legend()
    plt.savefig('./functions/psd_plot.png')
...
def MPL_crack_analysis(mpl_layer, csv_file, mpl=2, slice_idx=0, from_top=True, voxel_size=5):
    """
    ...
    plt.grid(True)
    plt.savefig('./functions/crack_size_distirbution.png')
    plt.close()
...
def plot_crack_labels(slice_image, crack_labels):
    ...
    plt.savefig('./functions/mpl_cracks_map.png')
    plt.close(fig)

def layer_pore_size_distribution(volume, gdl, visualize=True,voxel_size=5):
    mean_values = []
    sd_values = []
    pore_count_values = []
...
def MPL_heatmap(volume, mpl):
    ...
    plt.savefig('./functions/mpl_heatmap.png')
    plt.close(fig)
