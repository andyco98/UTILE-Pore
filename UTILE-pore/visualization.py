...
def visualize_volume(volume, case_name, capture = False):
   ...
   # Define color transfer function
    ...
    color_transfer_function.AddRGBPoint(3, 0.56, 0.0, 1.0)  # Other classes are green
...
def visualize_volume_realistic(volume):
    ...
    # Define opacity transfer function
    opacity_transfer_function = vtk.vtkPiecewiseFunction()
    opacity_transfer_function.AddPoint(0, 0.0001)  # Class 2 (now set to 0) is fully transparent
    opacity_transfer_function.AddPoint(1, 1.0)  # Class 1 is fully visible
...
#volume = tifffile.imread('C:/Users/a.colliard/Downloads/39bb2_HRNET_HRNET_multi_fusev2_dataset_noaug_-0.6808-23.keras.tif') #/ 255
...
#case_name = 'test'
#visualize_real_volume(volume, case_name, True)
#visualize_volume(volume, case_name, False)
